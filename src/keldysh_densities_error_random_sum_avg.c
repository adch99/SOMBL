...
void run(int runnum, struct SystemParams params, struct Dense densities);

int main(int argc, char ** argv)
{
    struct SystemParams params;
    struct OutStream outfiles;
    struct Dense densities;
    params_setup(argc, argv, &params, &outfiles, &argp, 0);
    params.energybins = 0;
    setup(&densities, params);
    int runnum;
    int maxruns = 100;
    for(runnum = 1; runnum <= maxruns; runnum++)
    {
        run(runnum, params);
    }
...
void run(int runnum, struct SystemParams params, struct Dense densities)
{
    ...
    char filename[256];
    char basename[64];
    snprintf(basename, 64, "altn_random_updown_n%d", runnum);
        ...
        for(alpha = 0; alpha < 2; alpha++)
        {
            for(beta = 0; beta < 2; beta++)
            {
                if (alpha == beta)
                {
                    ...
                    DTYPE * dsum = densities.dens[alpha][beta];
                    DTYPE * vsum = densities.vars[alpha][beta]; 
                    utils_add_to_matrix_real_error(dsum, density, params.num_sites, 1, vsum);
                ...
                else if (alpha != beta)
                {
                    ...
                    get_density(density, initial_cond, params, alpha, beta, bin, variance);
                    CDTYPE * dsum = densities.dens[alpha][beta];
                    CDTYPE * vsum = densities.vars[alpha][beta]; 
                    utils_add_to_matrix_complex_error(dsum, density, params.num_sites, 1, vsum);
                    // save_density(density, params, alpha, beta, bin, basename, variance);
                    free(variance);
                    free(initial_cond);
                    free(density);
...
void post_runs(int maxruns, struct SystemParams params, struct Dense * densities)
{
    int alpha, beta, i;
    DTYPE * dsum;
    DTYPE * vsum;
    for(alpha = 0; alpha < 2; alpha++)
    {
        dsum = densities->dens[alpha][alpha];
        vsum = densities->vars[alpha][alpha];
        for(i = 0; i < params.num_sites; i++)
        {
            *(dsum + i) /= (DTYPE) maxruns;
            *(vsum + i) /= (DTYPE) maxruns;
        }
    }
    ...
    for(alpha = 0; alpha < 2; alpha++)
    {
        dcsum = densities->dens[alpha][1-alpha];
        vcsum = densities->vars[alpha][1-alpha];
        for(i = 0; i < params.num_sites; i++)
        {
            *(dcsum + i) /= (CDTYPE) maxruns;
            *(vcsum + i) /= (CDTYPE) maxruns;
        }

        bin = -1;
        save_density(dcsum, params, alpha, beta, bin, basename, vcsum);
    }
...
void setup(struct Dense * d, struct SystemParams params)
{
    d->dens[0][0] = calloc(params.num_sites, sizeof(DTYPE));
    d->dens[1][1] = calloc(params.num_sites, sizeof(DTYPE));
    d->vars[0][0] = calloc(params.num_sites, sizeof(DTYPE));
    d->vars[1][1] = calloc(params.num_sites, sizeof(DTYPE));

    d->dens[0][1] = calloc(params.num_sites, sizeof(CDTYPE));
    d->dens[1][0] = calloc(params.num_sites, sizeof(CDTYPE));
    d->vars[0][1] = calloc(params.num_sites, sizeof(CDTYPE));
    d->vars[1][0] = calloc(params.num_sites, sizeof(CDTYPE));
}

void cleanup(struct Dense * d)
{
    int a, b;
    for(a = 0; a < 2; a++)
    {
