...
for coupling in $couplings; do
    for disorder in $disorders; do
        jobname="mbl_distvsgfuncsq_${size}x${size}_W${disorder}_C${coupling}_TU${hopup}_TD${hopdn}_N${runs}_BS${batchsize}"
        pbs_filename="jobs/${jobname}.pbs"
        log_filename="logs/${jobname}.log"
        execname="build/calculate_dist_vs_gfuncsq"
...
pwd
time ${execname} -n $runs -c $coupling -w $disorder -s $size -u $hopup -d $hopdn
date
